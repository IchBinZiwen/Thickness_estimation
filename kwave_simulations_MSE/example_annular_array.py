...
karray = kWaveArray(bli_tolerance=0.10, upsampling_rate=10)                         # generate a k-wave array as TX transducer (generic)
karray.add_annular_array(position = [15e-3,0,0], radius=15e-3, diameters = [[0, 5e-3], [5e-3,10e-3]], focus_pos = [20e-3,0,0]) # add something to the array, here: only one disc/circular element

source_sig = source_amp * tone_burst(1 / kgrid.dt, source_f0, source_cycles,
                                     signal_offset=np.round([0e-3, 10e-3/c0] / kgrid.dt).astype(int))        # define the TX signal
if debug_plot:
    plt.plot(source_sig)
